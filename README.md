...
- EU-first (headless email + password login), with structure for US/CA/AU
...
4. **Verify** — the app reads the vehicle's status; it only acts if the car is unlocked
...
- **On-device EU login** — sign in with your BlueLink email + password; the session is generated
  on-device via Hyundai's OneApp/CCI flow (no browser, no CAPTCHA) and auto-refreshes.
...
2. Open **Settings → Account** and either **Sign in** (email + password) or turn on
...
Before locking, AutoLock reads the vehicle's status and **skips** if the car is already locked,
the engine is running, a door/window is open, or the state is unknown.
...
- **Login** — the sign-in screen runs under `FLAG_SECURE` (no screenshots / recents preview).
  Your email + password are RSA-encrypted on-device and sent to Hyundai's IDP; the password is
  never stored — only the resulting encrypted session tokens are kept.
...
│     ├─ eu/      Real EU login (OneApp/CCI) + control flow
...
## License
Released under the [MIT License](LICENSE) © 2026 Vel-San. The reverse-engineered `client_id` /
`secret` / `stamp` constants in `RegionConfig` are public app constants (also present in the
reference projects), **not** anyone's Hyundai credentials.
